...
const ActivityCard: React.FC<ActivityCardProps> = ({ act, onSelect, active, fatherBackground, index }) => {
  let cost = act.cost;
  ...
  const cardBg = active
    ? 'bg-[rgba(212,175,55,0.12)] border-[#d4af37] shadow-[0_0_12px_rgba(212,175,55,0.15)]'
    : index % 2 === 0
      ? 'bg-slate-800/80 border-slate-700 hover:border-[#d4af37]/50 hover:bg-slate-750/90'
      : 'bg-slate-950/95 border-slate-850 hover:border-[#d4af37]/50 hover:bg-slate-950/100';

  return (
    <div
      onClick={() => onSelect(act.id)}
      className={`p-3.5 rounded-lg border cursor-pointer transition-all flex flex-col justify-between ${cardBg}`}
    >
